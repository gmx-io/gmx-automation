...
Web3Function.onRun(async (context: Web3FunctionEventContext) => {
  ...
  const fromTimestamp = await storage.get("fromTimestamp");
  ...
  await storage.set("distributionData", distributionData);

  return {
    canExec: true,
    message: `initiateDistribute transaction to be added`,
  };
});
...
async function getDistributionData(
  ...
  const referraldata = JSON.stringify(output, null, 4);
  return referraldata;
}
